...
void trigger_output() {
  ...
  if (settings_packet.data.sd_power_delay <= 0) {
    pulse_output_on_callback();
  } else {
    // turn on output to power the SDCard, do this delayed to give the Lora
    // packet time to trigger and boot the RPi
  ...
  if (status_packet.data.duration_of_pulse > 1900) {
    ...
    if (!timer_pulse.active()) {
      if (timer_debounce.active()) {
        ...
        // Let's see if our max timeout has elapsed, this is the maximum time we
        // will allow delaying the output trigger to prevent the output to be
        // delayed indefinitely.

        if (millis() - debounce_start > settings_packet.data.max_debounce_time * 1000 && settings_packet.data.max_debounce_time > 0) {
#ifdef debug
...
void status_measure_voltage() {
  ...
  stm32l0_battery =
      BAT_MON_CALIB * (value / 16) * (2500 / stm32l0_vdd);  // result in mV
...
#endif
  status_packet.data.battery = (uint8_t)stm32l0_battery;  // 0-5000 input, assuming 2500mV is minimum
                                                          // that is subtracted and then divided by 10
...
boolean status_send(void) {
  ...
  status_packet.data.system_functions_errors =
      status_packet.data.system_functions_errors & 0b00011111;
